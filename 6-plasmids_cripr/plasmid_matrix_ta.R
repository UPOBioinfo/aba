...
mtrx<-read.csv("ab_plasm_matrix_2112_84_48without0_ta_v3.tsv", sep = "\t", header = TRUE, row.names = "X")
...
colores <- c("grey95", "orange", "red2", "darkred")
pdf("../figures/SupplFigure5.pdf", width=10, height=6, paper='special')
pheatmap(mtrx, color = viridis(4), show_rownames=F, treeheight_row = 0, angle_col = 90, fontsize = 13, breaks = seq(-0.5,4,1),
         cluster_cols = TRUE, cluster_rows = FALSE, annotation_row=mtdt, annotation_colors = annotation_colors,
         legend_labels=c("No. TAs"))
dev.off()
