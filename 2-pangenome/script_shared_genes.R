...
# Gather data
data <- read.table("matrix_2467.tsv", comment.char="#", sep="\t")
...
# Metadata
mdata <- read.csv("metadata_file.tsv", sep="\t")
...
p <- ggplot(data=data_length, aes(ngenes, shared, label=id))+ 
  ...
  scale_x_continuous(breaks = seq(2000, 4500, by = 500), limits =c(2000, 4450))
p
...
pdf("../figures/Fig2.pdf", width=16, height=8, paper='special')
